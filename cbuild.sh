...
#  cbuild.sh -> cbuild.sh  //renamed to be unique file begining from E
#  Script for building CloverEFI source under GNU/Linux
...
#  Modified by mosquito on 1/30/15.
...
#declare -r NUMBER_OF_CPUS=$(sysctl -n hw.ncpu) # OSX command
declare -r NUMBER_OF_CPUS=$(grep processor /proc/cpuinfo|wc -l)
...
# Print the usage.
usage() {
    echo "Script for building CloverEFI sources on GNU/Linux"
    echo
